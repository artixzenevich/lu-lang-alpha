...
proc_def: PROC NAME COLON params? stmts END
params: NAME ("," NAME)*
...
// Вызовы процедур раскрываются ИЗНАЧАЛЬНО жадно: вызов может стоять только
// в конце выражения (один оператор перед ним), а его последний аргумент
// «съедает» весь хвост арифметики. Никакой неоднозначности не остаётся —
// парсеру не из чего выбирать. Пример: "1 + 2" в
//    печать: "x -> " + выполнить плюс_один: 1 + 2
// целиком попадает в аргумент (3 -> 4), а не в конкатенацию ("22").
// Сделать вызов «серединой» выражения можно только скобками:
...
def _resolve(node):
    """Раскрыть неоднозначности. Выбираем вариант, где вызов процедуры
    съедает как можно больше аргументов («выполнить ф: k - 1» — это
    вызов с аргументом k - 1, а не «вызов(k) − 1»), а «вернуть»
    предпочитает значение. Оставшиеся неоднозначности лексера (например,
    слово «или» можно прочитать как два токена «и» + «ли») решаем в пользу
    варианта, где на одной строке остаётся одна инструкция. Счёт только
    по тем местам, где варианты реально отличаются, чтобы один файл
    не «перетягивал» другой."""
...
def _pick(parses):
    ...
    # Главное — «жадные» аргументы вызова. Затем вариант, где на строке
    # меньше инструкций (то есть слово «или» не «располовинилось» на «и»
    # + переменную «ли»). Затем «вернуть» со значением, а напоследок —
    # больше аргументов в сумме (просто чтобы выбор был стабильным).
    return max(
        parses,
        key=lambda t: (
            local_score(t),
